...
final class TableColumn<K extends Comparable<K>, T> extends ReadOnlyTableColumn<K, T> {
  ...
  Widget build(BuildContext context, T item, int index) {
    return InkWell(
      ...
      child: cellBuilder(context, item, index),
    );
  }
...
final class TableColumnIconButton<K extends Comparable<K>, T> extends ReadOnlyTableColumn<K, T> {
  final IconData icon;
  final void Function(T item, int index) onPressed;
  final Color? iconColor;
  final double? iconSize;

  const TableColumnIconButton({
    required this.icon,
  ...
  @override
  int get hashCode => Object.hash(id, size, title, icon, iconColor, iconSize, format);

  @override
  bool operator ==(Object other) =>
      other is TableColumnIconButton<K, T> &&
      other.icon == icon &&
